##############################
# llg admin api backend
##############################

module "llg-admin-api-backend" {
  source      = "../../modules/service/backend"
  module_name = local.llg-admin-api.module-name
  ...
  service_port   = local.llg-admin-api_service_port
  ...
  ecs_cluster = {
    name = module.CICD.admin_api_ecs_cluster.name
    id   = module.CICD.admin_api_ecs_cluster.id
  }
  ecs_task_execution_role_arn = module.CICD.ecs_task_execution_role.arn
  ecs_task_role_arn           = module.CICD.ecs_task_role.arn
  ecs_options                 = local.llg-admin-api.ecs_options
  github_info                 = local.llg-admin-api.github_info
  lb_tg_health_check_options  = local.llg-admin-api.lb_tg_health_check_options
...
#  rds_port                    = module.mysql.rds.port
  ...
  sub_domain                  = local.llg-admin-api.module-name
  ...
  environment_variables = {
    s3_project_folder = {
      ...
      value = local.llg-admin-api.module-name
      type  = "PLAINTEXT"
    }
    project_start_script_name = {
      name  = "PROJECT_START_SCRIPT_NAME"
      value = "start_admin:prod"  
      type  = "PLAINTEXT"
    }
  }

  codepipeline_trigger_branches = [
    "${local.llg-admin-api.github_info.branch}"
  ]
...
locals {
  llg-admin-api_service_port      = 5100
  llg-admin-api_health_check_path = "/health"
  llg-admin-api = {
    ...
    ecs_options = {
      ...
      healthCheck = {
        command = [
          "CMD-SHELL",
          "node healthCheck.js ${local.llg-admin-api_service_port} ${local.llg-admin-api_health_check_path} || exit 1"
        ],
        "interval" = 30
        "timeout"  = 5
        "retries"  = 3
    ...
    lb_tg_health_check_options = {
      ...
      timeout             = 5
      interval            = 30
      path                = local.llg-admin-api_health_check_path
      port                = "traffic-port"
      protocol            = "HTTP"
    }
  }
...
output "service_url_llg-admin-api-backend" {
  value = module.llg-admin-api-backend.service_domain.fqdn
}
