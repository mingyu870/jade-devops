# notify setting
resource "aws_codestarnotifications_notification_rule" "ssr_web-notify" {
  detail_type    = "FULL"
  event_type_ids = local.notify_events
  name           = "${var.full_proj_name}-${var.module_name}-notify"
  resource       = aws_codepipeline.ssr_web.arn
...
resource "aws_codestarnotifications_notification_rule" "ssr_web-notify-all" {
  detail_type    = "FULL"
