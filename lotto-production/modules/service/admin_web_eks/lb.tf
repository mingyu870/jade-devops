...
resource "aws_lb_target_group" "admin_web_eks" {
...
resource "aws_lb_listener" "admin_web_eks_http" {
  load_balancer_arn = aws_lb.admin_web_eks.arn
  port              = 80
  protocol          = "HTTP"
...
resource "aws_lb_listener" "ssr_web_https" {
  ...
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.admin_web_eks.arn
  }
}
