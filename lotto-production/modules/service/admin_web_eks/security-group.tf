resource "aws_security_group" "admin_web_eks_services" {
  ...
  ingress {
    ...
    protocol        = "tcp"
    security_groups = [aws_security_group.admin_web_eks_services_alb.id]
  }
...
# alb
resource "aws_security_group" "admin_web_eks_services_alb" {
...
locals {
  sg_ingress_rule_description = "allow from ${var.full_proj_name} ${var.module_name} eks"
}
...
resource "aws_vpc_security_group_ingress_rule" "http" {
  ...
  referenced_security_group_id = aws_security_group.admin_web_eks_services.id
  from_port                    = 80
  ip_protocol                  = "tcp"
  to_port                      = 80
  tags = {
