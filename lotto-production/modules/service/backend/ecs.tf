resource "aws_ecs_service" "backend" {
  ...
  load_balancer {
    ...
    container_name   = aws_ecr_repository.backend.name
  ...
  container_definitions = jsonencode([
    {
      ...
      logConfiguration = {
        ...
        options : {
          ...
          awslogs-group : "/ecs/${var.ecs_cluster.name}/${local.ecs_service_name}"
      ...
      environment = [
        ...
        },
        {
          ...
          value = "25fd0c4c2d814f44ff66ad50745e1875FFFFNRAL"
        },
        {
          name  = "NEW_RELIC_LOG"
          value = "warn"
...
resource "aws_appautoscaling_target" "ecs_target" {
  min_capacity       = 3
  max_capacity       = 6
  resource_id        = "service/${var.ecs_cluster.name}/${aws_ecs_service.backend.name}"
...
resource "aws_cloudwatch_metric_alarm" "cpu_alert_scale_out" {
  alarm_name                = "${var.ecs_cluster.name}-cpu-alert-scale-out"
...
resource "aws_cloudwatch_metric_alarm" "cpu_alert_scale_in" {
  alarm_name                = "${var.ecs_cluster.name}-cpu-alert-scale-in"
